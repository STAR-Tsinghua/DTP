#!/usr/bin/python3
import numpy as np
import matplotlib
matplotlib.use("Agg")
import matplotlib.pyplot as plt
import matplotlib.ticker as mtick
from matplotlib.pyplot import MultipleLocator
#从pyplot导入MultipleLocator类，这个类用于设置刻度间隔

deadline=200
block_num=600
fig, ax = plt.subplots(dpi=200)#figsize=(16, 12)
case=['DTP','QUIC','Deadline','Priority']
case_draw=['DTP','QUIC','DTP-D','DTP-P']
line_style = ['-','--','-.',':']
x = np.arange(1.0,21.0,1.0)
y = np.arange(1.0,21.0,1.0)

def good_bytes_list(f):
    goodbytes = [0]*block_num
    for line in f.readlines():
        if line.find('goodbytes') != -1:
            # print(line)
            s_arr = line.split()
            id = int(s_arr[4])
            good_bytes_add = int(s_arr[6])
            # if id == 4:
            #     print(id,int(id/4)-1,good_bytes_add)
            #     print(line)
            #     print(goodbytes)
            goodbytes[int(id/4)-1] += good_bytes_add
            # print(line)
            # break
    return goodbytes



for i in range(len(case)):
    c=case[i]
    c_draw=case_draw[i]
    ls=line_style[i]
    # fpath='result/data/'+c+'/'+c+'-square-wave.log'
    # f=open(fpath,'r')
    # f.readline()# 第一行：StreamID goodbytes bct BlockSize Priority Deadline
    # Block = [999 for n in range(block_num)]
    # # 文件格式：每一行第一个参数为ID，第三个参数为离发出的ms
    # for line in f.readlines():
    #     arr=line.split()
    #     block_id=int(arr[0])
    #     block_id=int(block_id/4-1)
    #     through=int(arr[1])
    #     Block[block_id]=through
    fpath = 'result/server_log/'+c+'-server-square-wave.log'
    f =  open(fpath,'r')
    Block = good_bytes_list(f)
    f.close()
    sum=0
    i=0
    for id in range(block_num):
        sum+=Block[id]
        # 30 blocks each sec
        if (id+1)%30==0:
            y[i]=sum/1000_000
            i+=1
            sum=0
    ax.plot(x,y,label=c_draw,linestyle=ls,linewidth=2)


x1 = np.linspace(0,20,1000)
y1 = np.linspace(0,0,1000)
y2 = np.linspace(0,0,1000)
for i in range(x1.size):
    if (x1[i])%10<5:
        y1[i]=12
        # if (x1[i])%5<0.02:
        #     plt.text(x1[i],y1[i]+1,"12MBps",fontsize='xx-large')
    else:
        y1[i]=4
        # if x1[i]%5<0.02:
        #     plt.text(x1[i],y1[i]+1,"4MBps",fontsize='xx-large')

ax.fill_between(x1,y1,y2,color='#000000',where=y1>y2,interpolate=True,alpha=0.1)
# 设置百分比显示

# ax.set_title('Bandwidth Square Wave-Good Bytes')
ax.set_xlabel('Time/s',size='xx-large')
ax.set_ylabel('Good bytes/MB',size='xx-large')
plt.xlim(0)
plt.ylim(0,10)
ax.legend(fontsize='x-large')
ax.set_xticks(x)

x_major_locator=MultipleLocator(5)
ax.xaxis.set_major_locator(x_major_locator)

plt.xticks(size='xx-large')
plt.yticks(size='xx-large')
plt.savefig('new_goodbytes_bandwidth_square_wave.png',bbox_inches='tight') # 核心是设置成tight






