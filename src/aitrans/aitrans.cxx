...
uint64_t CSelectBlock(char *blocks_string, uint64_t block_num,
    ...
    blocks.block_size = (uint64_t *)malloc(block_num * sizeof(uint64_t));;
    for (uint64_t i = 0; i < block_num; i++) {
        ...
        blocks.block_size[i] = strtoull(blocks_string, &blocks_string, 10);
        fprintf(stderr,"block_id: %lu, deadline: %lu, priority: %lu, create_time: %lu, size: %lu\n",blocks.blocks_id[i], blocks.blocks_deadline[i],
            blocks.blocks_priority[i], blocks.blocks_create_time[i], blocks.block_size[i]);
    }
    /*********** Player's Code Here*************/
